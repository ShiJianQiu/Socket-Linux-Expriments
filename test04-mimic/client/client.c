...
{  
    ...
    server_addr_clint.sin_port = htons(SERVER_PORT_MASTER);  
    ...
    while(1)  
    {
        ...
        STDOUT_FILENO：向屏幕输出
        */
        FD_SET(server_master_sock_fd, &client_fd_set);
        FD_SET(server_mimic_sock_fd, &client_fd_set);
        ...
        */
        select(max(server_master_sock_fd,server_mimic_sock_fd) + 1, &client_fd_set, NULL, NULL, &tv);
        if(FD_ISSET(STDIN_FILENO, &client_fd_set))  //用于测试指定的文件描述符是否在该集合中。
        {  
            bzero(input_msg, BUFFER_SIZE);  //置字节字符串s的前n个字节为零且包括‘\0’
            fgets(input_msg, BUFFER_SIZE, stdin);  
            if(send(server_master_sock_fd, input_msg, BUFFER_SIZE, 0) == -1)  
            {  
                perror("发送消息出错!\n");  
            }  
        ...
        if(FD_ISSET(server_master_sock_fd, &client_fd_set))  
        {  
            bzero(recv_msg, BUFFER_SIZE);  
            long byte_num = recv(server_master_sock_fd, recv_msg, BUFFER_SIZE, 0);  
            if(byte_num > 0)  
