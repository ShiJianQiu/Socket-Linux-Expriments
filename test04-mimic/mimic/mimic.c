...
int main(int argc, const char *argv[])
{
    ...
    if(connect(server_master_sock_fd, (struct sockaddr *)&server_addr_master, sizeof(struct sockaddr_in)) == 0)  { 
        printf("connect with a master, server_master_sock_fd = %d\n",server_master_sock_fd);
        //fd_set
                ...
                if (FD_ISSET(server_master_sock_fd, &server_fd_set)){
                    ...
                    {  
                        ...
                        */
                        ...
                        // printf("服务器:%s\n", recv_msg); 
                        long byte_num_mimic = 0,tar=__INT_MAX__;
                        int cnt=0;
                        while(cnt<=1){
                            for (int i = 0; i < CONCURRENT_MAX; i++)
                            {
                                /* code */
                                if (calculator_fds[i] != 0 && send_to_calculator[i].receive_len > 0&& send_to_calculator[i].statue==OK){
                                    //谁先谁正确
                        ...
                        //     }
                        // }
                        send(server_master_sock_fd,recv_msg,sizeof(recv_msg),0);
                        printf("返回Master端:%s", recv_msg);
